...
extern BOOL CheckCollMaruToMaru(MARU maru1, MARU maru2);		//点と円の当たり判定

extern RECT GetRect(int left, int top, int right, int bottom);	//RECT型を一時的に渡す
extern void DrawRect(RECT r, unsigned int color, bool b);		//RECTを利用して四角を描画する
extern VOID DrawMaru(MARU c, unsigned int color, bool b, int thick = 1);	//MARUを利用して円を描画する
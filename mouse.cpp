...
VOID MouseUpdate(VOID)
{
	...
	//左ボタンを押しているかチェック（TRUEは0以外なので２も４もTRUE)
	...
	//中ボタンを押しているかチェック（TRUEは0以外なので２も４もTRUE)
	...
	//右ボタンを押しているかチェック（TRUEは0以外なので２も４もTRUE)
...
/// </summary>
/// <param name="MOUSE_INPUT">MOUSE_INPUT_</param>
...
BOOL MouseDownKeep(int MOUSE_INPUT_, int milliTime)
{
	...
	//押し続ける時間=秒数×FPS値
	//例）60FPSのゲームで、1秒間押し続けているなら、1秒×60FPS
...
BOOL MouseClick(int MOUSE_INPUT_)
{
	if (mouse.OldButton[MOUSE_INPUT_] >= 1	//直前は押していて
		&& mouse.Button[MOUSE_INPUT_] == 0)	//今は押していないとき
	{
		return TRUE;	//マウスをクリックした（押し続けても1回のみ発生）
	}
	else
	{
